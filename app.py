# -*- coding: utf-8 -*-
# Baccarat Master Ultimate - Precision 15 Apex · Full UI
# 只加不减：集成 13/14 全部功能 + 稳定性修复 + 决策增强
...
st.markdown('<h1 class="main-header">🐉 百家乐大师 Precision 15 Apex · Full UI</h1>', unsafe_allow_html=True)
...
class GameStateDetector:
    ...
    def detect(roads):
        ...
        al=GameStateDetector._detect_multi_路_alignment(roads) if False else None
        # 为保持一致性，直接调用：
...
def road_recommendation(roads):
    ...
    if roads['small_road']:
        r=roads['small_路'].count('R') if False else roads['small_road'].count('R')
...
def enhanced_export_data():
    ...
    st.download_button(label="📥 下载完整数据",
                       data=json_str,
...
def display_complete_analysis():
    ...
    st.markdown("### 🧠 智能决策引擎（动态阈值 + 投票兜底 + HOLD自适应）")
    ...
    with col2: st.metric("熵值", f"{ent:.3f}")
    with col3: st.metric("趋势", f"{trend:.3f}")
...
def display_complete_roads():
    ...
    if roads['three_bead_road']:
        st.markdown("#### 🔶 三珠路")
        for i,g in enumerate(roads['three_bead_路'][-6:]) if False else enumerate(roads['three_bead_road'][-6:]):
...
def main():
    with st.sidebar:
        st.markdown("## ⚙️ 控制台")
        st.caption("随时调整 EOR 副数；AI 权重后台自学习（界面只读显示）。")
