# -*- coding: utf-8 -*-
# Baccarat Master Ultimate - Precision 14 Fusion Touch v3 Ultimate Extended
# 只加不减：在 v14 基础上启用 Tie/Pair 概率预测 + 触控输入精简一排 + 保留所有功能
...
st.set_page_config(page_title="🐉 百家乐大师 Precision 14 Fusion Touch v3", layout="centered")
...
.sidebet-panel{background:#0f172a;border:1px solid #334155;border-radius:10px;padding:12px;color:#e5e7eb;}
.sidebet-badge{display:inline-block;padding:4px 8px;border-radius:8px;margin-right:6px;font-weight:700;}
.sb-tie{background:#334155;color:#e5e7eb;}
.sb-pair{background:#1d4ed8;color:#e0e7ff;}
.small-btn{font-size:13px;padding:6px 8px;}
</style>
""", unsafe_allow_html=True)

st.markdown('<h1 class="main-header">🐉 百家乐大师 Precision 14 Fusion Touch v3</h1>', unsafe_allow_html=True)
...
# ===== 新增：旁注统计（Tie / Pair） =====
if "side_bet_stats" not in st.session_state:
    st.session_state.side_bet_stats = {
        "tie_count": 0, "ppair_count": 0, "bpair_count": 0, "total_hands": 0,
        "recent_tie": deque(maxlen=200),
        "recent_ppair": deque(maxlen=200),
        "recent_bpair": deque(maxlen=200)
    }
...
# ========================== 新增：旁注概率估计（Tie / Pair） ==========================
def _pair_flag_from_cards(cards_list):
    """从两张起手牌判断是否对子（仅限牌点输入模式时可判定）"""
    if not cards_list or len(cards_list) < 2: return None
    r1, r2 = cards_list[0], cards_list[1]
    return (r1 == r2)

def estimate_side_bets(seq, metrics, side_stats, last_game):
    """
    返回：tie_prob, p_pair_prob, b_pair_prob （已限制在合理范围）
    基线（参考 8 副牌）：Tie≈9.5%， Player Pair≈7.5%， Banker Pair≈7.5%
    """
    base_tie = 0.095
    base_pair = 0.075

    z = float(metrics.get('z',0))
    mom = float(metrics.get('momentum',0))
    ent = float(metrics.get('entropy',0))
    cusum = float(metrics.get('cusum',0))

    # —— Tie：高熵、低动量、低Z 的环境略增；强趋势时下修
    tie_score = (ent*0.6 + (1-abs(mom))*0.25 + (1-abs(z))*0.15)  # 0~1
    tie_prob = base_tie * (1 + 0.6*(tie_score - 0.5))            # +-30% 浮动
    if abs(cusum) > 0.15:
        tie_prob *= 0.9  # 趋势明显时降低一点
    tie_prob = float(np.clip(tie_prob, 0.05, 0.14))

    # —— Pair：基于最近真实对子出现频率微调；若无牌点输入则用基线
    window = 120
    rec_ppair = list(side_stats['recent_ppair'])
    rec_bpair = list(side_stats['recent_bpair'])
    w_p = sum(rec_ppair[-window:]) / max(1, len(rec_ppair[-window:])) if rec_ppair else 0
    w_b = sum(rec_bpair[-window:]) / max(1, len(rec_bpair[-window:])) if rec_bpair else 0

    p_pair_prob = base_pair * (1 + 0.5*(w_p - base_pair))  # 轻微均值回归
    b_pair_prob = base_pair * (1 + 0.5*(w_b - base_pair))

    # 牌点最新一局若出现对子，下一局概率略下修（避免追高）
    if last_game and last_game.get('mode')=='card':
        if _pair_flag_from_cards(last_game.get('player_cards')): p_pair_prob *= 0.9
        if _pair_flag_from_cards(last_game.get('banker_cards')): b_pair_prob *= 0.9

    p_pair_prob = float(np.clip(p_pair_prob, 0.035, 0.12))
    b_pair_prob = float(np.clip(b_pair_prob, 0.035, 0.12))

    return tie_prob, p_pair_prob, b_pair_prob

def sidebet_suggestion(prob, kind="tie"):
    # 简单阈值策略，仅作展示，不影响主流程
    if kind=="tie":
        if prob >= 0.12: return "⚠️ 可小注搏和"
        if prob >= 0.105: return "🙂 可考虑"
        return "🚫 不建议"
    else:
        if prob >= 0.10: return "⚠️ 可小注搏对子"
        if prob >= 0.085: return "🙂 可考虑"
        return "🚫 不建议"
...
def _update_side_bets_stats(game):
    stats = st.session_state.side_bet_stats
    stats['total_hands'] += 1
    # Tie 来自结果
    if game['result'] == 'T':
        stats['tie_count'] += 1
        stats['recent_tie'].append(1)
    else:
        stats['recent_tie'].append(0)
    # Pair 仅在牌点模式可判断：前两张是否同点
    if game.get('mode')=='card':
        p_pair = _pair_flag_from_cards(game.get('player_cards'))
        b_pair = _pair_flag_from_cards(game.get('banker_cards'))
        if p_pair: 
            stats['ppair_count'] += 1
            stats['recent_ppair'].append(1)
        else:
            stats['recent_ppair'].append(0)
        if b_pair:
            stats['bpair_count'] += 1
            stats['recent_bpair'].append(1)
        else:
            stats['recent_bpair'].append(0)
    else:
        # 无法判断时记0，不影响历史比例
        stats['recent_ppair'].append(0)
        stats['recent_bpair'].append(0)
...
def handle_quick_input(quick_banker, quick_player, quick_tie):
    if quick_tie:
        res='T'
    else:
...
def handle_batch_input(batch_input):
    s=(batch_input.upper()
       .replace('庄','B').replace('闲','P').replace('和','T')
       .replace(' ',''))
...
# ============ 新：触控输入（精简一排 + 保留键盘 & 快捷 B/P/T） ============
def small_rank_buttons(key_prefix):
    cols = st.columns(len(RANKS))
    pressed = []
    for i, r in enumerate(RANKS):
        if cols[i].button(r, key=f"{key_prefix}_{r}_{datetime.now().timestamp()}", help=f"输入 {r}", use_container_width=True):
            pressed.append(r)
    return pressed

def build_touch_row(label, text_key, touch_key):
    st.caption(label)
    val = st.text_input("", key=text_key, placeholder="如：K10 或 552")
    # 小一排触控
    with st.container():
        cols = st.columns(len(RANKS))
        for i, r in enumerate(RANKS):
            if cols[i].button(r, key=f"{touch_key}_{r}", help=f"{label} 添加 {r}", use_container_width=True):
                cur = st.session_state.get(text_key,"")
                sep = "" if (cur.strip()=="" or cur.strip()[-1].upper()=="0") else ""
                st.session_state[text_key] = (cur + r).upper()
    return st.session_state.get(text_key, val)
...
def display_complete_interface():
    ...
    if st.session_state.input_mode=='card':
        st.info("📱 触控一排 + 键盘双输入；建议先点按再微调。")
        col1,col2 = st.columns(2)
        with col1:
            p_input = build_touch_row("闲家牌", "player_card", "touch_p")
        with col2:
            b_input = build_touch_row("庄家牌", "banker_card", "touch_b")
        ...
        st.info("💡 快速模式：直接记录结果（含和局 T），用于快速看路分析")
        ...
        st.markdown("### 📝 批量输入")
        batch=st.text_input("输入序列（支持 庄闲和 / BPT）", placeholder="如：BPBBPT 或 庄闲庄庄和")
        if st.button("✅ 确认批量输入", use_container_width=True) and batch:
            handle_batch_input(batch)
        if qb or qp or qt: handle_quick_input(qb,qp,qt)
...
def display_complete_analysis():
    ...
    c1.metric("动态阈值", f"{threshold:.3f}")
    c2.metric("熵值", f"{ent:.3f}")
    c3.metric("趋势强度", f"{trend:.3f}")
    c4.metric("HOLD调整", f"{hold_adjustment:.2f}")
    if vote_override:
    ...
    # ===== 新增：旁注概率展示（Tie / Player Pair / Banker Pair） =====
    st.markdown("#### 🎲 旁注概率（实验，仅供参考）")
    tie_p, ppair_p, bpair_p = estimate_side_bets(seq, metrics, st.session_state.side_bet_stats,
                                                 st.session_state.ultimate_games[-1] if st.session_state.ultimate_games else None)
    s1,s2,s3 = st.columns(3)
    with s1:
        st.markdown(f"""<div class="sidebet-panel">
            <span class="sidebet-badge sb-tie">TIE</span>
            <b>{tie_p*100:.1f}%</b><br/>{sidebet_suggestion(tie_p,'tie')}
        </div>""", unsafe_allow_html=True)
    with s2:
        st.markdown(f"""<div class="sidebet-panel">
            <span class="sidebet-badge sb-pair">P Pair</span>
            <b>{ppair_p*100:.1f}%</b><br/>{sidebet_suggestion(ppair_p,'pair')}
        </div>""", unsafe_allow_html=True)
    with s3:
        st.markdown(f"""<div class="sidebet-panel">
            <span class="sidebet-badge sb-pair">B Pair</span>
            <b>{bpair_p*100:.1f}%</b><br/>{sidebet_suggestion(bpair_p,'pair')}
        </div>""", unsafe_allow_html=True)
    st.caption("※ 旁注概率以保守模型估计，主要用于提示“高概率时机”，不影响主投注逻辑。")
...
def display_professional_stats():
    ...
    # 旁注长期统计
    sb = st.session_state.side_bet_stats
    st.markdown("#### 🎯 旁注长期统计")
    e1, e2, e3, e4 = st.columns(4)
    e1.metric("总局数", sb['total_hands'])
    e2.metric("和局占比", f"{(sb['tie_count']/sb['total_hands']*100 if sb['total_hands'] else 0):.1f}%")
    e3.metric("闲对占比(估)", f"{(sum(sb['recent_ppair'])/max(1,len(sb['recent_ppair']))*100):.1f}%")
    e4.metric("庄对占比(估)", f"{(sum(sb['recent_bpair'])/max(1,len(sb['recent_bpair']))*100):.1f}%")
...
def enhanced_export_data():
    data = {
        ...
        'side_bet_stats': {
            **st.session_state.side_bet_stats,
            "recent_tie": list(st.session_state.side_bet_stats["recent_tie"]),
            "recent_ppair": list(st.session_state.side_bet_stats["recent_ppair"]),
            "recent_bpair": list(st.session_state.side_bet_stats["recent_bpair"]),
        },
...
def show_quick_start_guide():
    if len(st.session_state.ultimate_games) == 0:
        ...
        <h3>🎯 快速开始指南</h3>
        <p>1. 选择「牌点输入」或「快速看路」开始记录</p>
        <p>2. 记录≥3局后自动激活AI分析</p>
        <p>3. 🔔 旁注概率（和局/对子）已启用，谨慎使用</p>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    ...
    with c1:
        if st.button("🔄 开始新牌靴", use_container_width=True):
            ...
            st.session_state.side_bet_stats = {
                "tie_count": 0, "ppair_count": 0, "bpair_count": 0, "total_hands": 0,
                "recent_tie": deque(maxlen=200),"recent_ppair": deque(maxlen=200),"recent_bpair": deque(maxlen=200)
            }
